...
- 数据透视与维度：指标行（指标名 + 行内 `✖` 移除；指标卡的行前面带 `第 N 张 · `）+ `＋ 从模型字典选择`（图表 / 环形图 / 数据明细）或 `＋ 添加指标卡`（指标卡）；未绑定字段时提示「⚠ 尚未绑定字段，画布无法出图」。**数量上限按「组件类型 × 口径」判定**（见下表）：多选组件在按钮旁实时显示 `N / 5 个`，达上限时按钮置灰并附一行「已达上限 5 个，先移除一个再加。」；指标卡显示 `N / 6 张`，满 6 张提示「已达上限 6 张，先移除一张再加。」；只允许 1 个指标的组件（环形图＝单机与机群一致；折线图＝仅机群口径）显示「1 个（单机口径）」/「1 个（机群口径）」，按钮不置灰、可反复点开换指标。
- PDF 物理排版：`纸张宽度占比`（`100%（满宽整行）` / `50%（半宽并排）`）、`渲染完毕后强制换页`（勾选框）、`🗑 删除当前组件`。
- 文本段落（仅文本段落）：`段落内容`（多行文本，占位「输入报表的文字说明、导言或总结分析...」）。

| 可视化类型 | 单机口径 | 机群口径 | 说明（报表语义） |
| --- | --- | --- | --- |
| 折线图 | **≤ 5 个指标**（须同单位，5 色循环） | **只能 1 个指标** | X 轴＝时间序列；单机一个指标一条线、5 色循环，同屏指标共用同一套 Y 轴刻度；机群每台设备一条线（同一指标、同量纲对比，最多画前 10 台） |
| 柱状图 | **≤ 5 个指标**（须同单位，同一时间块内并排，5 色循环） | **≤ 5 个指标**（须同单位，每台设备一组条） | 单机多指标并排；机群排行榜多指标并排（走 5 色循环）、最多画前 10 台；右栏可切排布方向（垂直＝时间序列 / 水平＝设备排行） |
...
- 三栏级联：`设备类型`（14 种设备类型，机群口径的物模型列改为「报表」）→ `物模型` → `字段`；字段列右侧显示单位徽标；表头随组件与口径显示「字段（单选 1 个）」（环形图＝单机与机群；折线图＝仅机群口径）/「字段（可多选 · 最多 5 个）」（**单机折线图**、**柱状图（单机与机群）**）/「字段（可多选 · 一个指标一列）」（数据明细）/「字段（每次选 1 个 · 加 1 张卡）」（指标卡），右上角显示字段个数。
- 选择控件：**环形图（单机与机群一致）与机群折线图为单选框**（只能 1 个，再选自动替换、再点一次可取消）；**单机折线图**与**柱状图（单机与机群）**为**复选框**（多选 ≤ 5 个，须同单位）；**数据明细**为复选框（多选列，不设上限）；指标卡为复选框累加；基础字段「统计周期 / 设备机型 / 设备编号」始终置灰不可选，指标卡中已加过的字段同样置灰。
- **绑定拦截（数量：单机 / 机群有区别；字段匹配：不区分口径）**：
  - 数量超限（当前多选＝**单机折线图**、**柱状图（单机与机群）**与**数据明细**）：多选组件选满上限后**其余未选字段置灰不可点**（悬停提示「已达上限 5 个指标，先取消一个再选」），勾选动作被撤销，并提示「已达上限 5 个指标（5 色循环，第 6 个会无法区分颜色），请先取消一个再选。」；底部计数显示「已选 N / 5 个 · 同单位多指标并排」。
  ...
  - 单指标替换（**环形图＝单机与机群；折线图＝仅机群口径**）：选中新指标即替换原有的，并提示「只支持 1 个指标：「工时」已替换为「能耗」」；「全选当前模型」在该类组件下隐藏，多选组件点全选超限时提示「该组件最多选 5 个指标，全选会超限，请逐个点选。」。
  - 连带规则：切换可视化类型时按新类型的上限裁剪（数据明细 3 列 → 环形图与机群折线图只留第 1 个并提示；→ 单机折线图 / 柱状图保留 ≤ 5 个）；读取已保存的版式 JSON 时同样按「组件类型 × 口径」重新裁剪。
...
| 指标数量 | 折线图：**单机 ≤ 5 个**（须同单位、5 色循环）、**机群只能 1 个**；柱状图：**两个口径均 ≤ 5 个**（须同单位、5 色循环，机群为设备排行）；环形图：**只能 1 个指标**（单机与机群一致，单选框、再选即替换）；指标卡 ≤ 6 张（一个字段一张卡）；数据明细 ≥ 1 列、不限列数 |
...
- 弹窗标题「配置『我需要更多报表』候选报表」，逐行维护候选项，每行字段：预览图（非必传，点击上传 / 替换，右上角 ✕ 移除）、报表名称*、一句话描述、删除行（**无标签字段**——配置侧不维护标签，端侧徽标固定显示 `可申请`；标签字段已于 2026-09-24 删除）；底部「＋ 添加一项」+ 项数统计 + 取消 / 保存配置；空态「清单为空，点左下角『＋ 添加一项』开始配置」。
...
- 候选报表（可多选）：优先读取后台「机群需求反馈」配置的清单；未配置时用内置默认清单（6 项，标签**全部为 `可申请`**：超时工作 / 设备异常通知 / 电池电量 / 碳排放 / 驾驶员行为评分 / 备件与保养消耗）。
- 候选项展示：每项含复选框、预览图缩略图、报表名称 + 标签 `可申请`（红）与一句话描述；点击缩略图放大查看大图（点任意处或按 Esc 关闭）；未上传预览图的候选项同样列出，但不展示预览图。
...
- 候选报表（可多选，默认 6 项）：缩略图 + 名称 + 标签 + 描述（名称 / 标签 / 描述来自后台候选清单配置，未配置时用内置默认；标签**统一为 `可申请`**）。
...
- 标签文案**统一为 `可申请`**（「已规划」状态已删除），按界面语言展示（未配置的语言回落简体中文）。
