...
- 基本信息分两行排布：**第 1 行 = 模版名称 / 模版描述**（两个输入框各占一半宽度，避免被挤窄）；**第 2 行 = 启用此模版 / 整体口径**。
- 启用此模版：第 2 行左侧，开关右侧跟一枚状态文案（`已启用` / `已停用`，与列表「启用」列口径一致，随开关即时切换），切换后随保存生效，**不随步骤切换而隐藏**。
- 整体口径：第 2 行右侧的 `单机设备` / `机群（聚合）` 分段切换，是**整份报表（模版）级**的属性，**不放在第 2、3 步的组件里**；切换即同时切换第 2 步的数据列配置与第 3 步 PDF 版式设计器的数据模块与版式（两套配置分别保存、互不影响）。
...
- 数据透视与维度：指标行（指标名 + 行内 `✖` 移除；指标卡的行前面带 `第 N 张 · `）+ `＋ 从模型字典选择`（图表 / 环形图 / 数据明细）或 `＋ 添加指标卡`（指标卡）；未绑定字段时提示「⚠ 尚未绑定字段，画布无法出图」。**数量上限按「组件类型 × 口径」判定**（见下表）：多选组件在按钮旁实时显示 `N / 5 个`，达上限时按钮置灰并附一行「已达上限 5 个，先移除一个再加。」；指标卡显示 `N / 6 张`，满 6 张提示「已达上限 6 张，先移除一张再加。」；只允许 1 个指标的组件（机群折线 / 环形）显示「1 个（机群口径）」。
- PDF 物理排版：`纸张宽度占比`（`100%（满宽整行）` / `50%（半宽并排）`）、`渲染完毕后强制换页`（勾选框）、`🗑 删除当前组件`。
- 文本段落（仅文本段落）：`段落内容`（多行文本，占位「输入报表的文字说明、导言或总结分析...」）。

| 可视化类型 | 单机口径 | 机群口径 | 说明（报表语义） |
| --- | --- | --- | --- |
| 折线图 | **≤ 5 个指标**（须同单位，5 色循环） | **只能 1 个指标** | X 轴＝时间序列；一个指标一条线、5 色循环，同屏指标共用同一套 Y 轴刻度；机群每台设备一条线（同一指标、同量纲对比） |
| 柱状图 | **≤ 5 个指标**（须同单位，同一时间块内并排） | **≤ 5 个指标**（须同单位，每台设备一组条） | 右栏可切排布方向（垂直＝时间序列 / 水平＝设备排行） |
| 环形图 | **≤ 5 个指标**（须同单位；整环 = 所选指标合计、图例分块 = 每个指标） | **只能 1 个指标** | 只算所选周期的一个总数，不按时间画点。单机：一个饼按状态切开，看工作 / 怠速 / 关机各占多少；机群：一个饼按设备切开，看每台设备各占多少（只画前 5 台，后面的并成第 6 块灰色「其他」），而且只能选能累加的指标（工时 / 油耗这类），开关机状态不能选 |
...
- 三栏级联：`设备类型`（14 种设备类型，机群口径的物模型列改为「报表」）→ `物模型` → `字段`；字段列右侧显示单位徽标；表头随组件与口径显示「字段（单选 1 个）」（单指标组件）/「字段（可多选 · 最多 5 个）」（单机图表）/「字段（可多选 · 一个指标一列）」（数据明细）/「字段（每次选 1 个 · 加 1 张卡）」（指标卡），右上角显示字段个数。
- 选择控件：**单机**折线 / 柱状 / 环形与数据明细为**复选框**（多选），**机群**折线 / 环形为**单选框**（只能 1 个，再选自动替换、再点一次可取消），指标卡为复选框累加；基础字段「统计周期 / 设备机型 / 设备编号」始终置灰不可选，指标卡中已加过的字段同样置灰。
- **指标数量拦截（单机 / 机群有区别）**：
  - 数量超限：多选组件选满上限后**其余未选字段置灰不可点**（悬停提示「已达上限 5 个指标，先取消一个再选」），勾选动作被撤销，并提示「已达上限 5 个指标（5 色循环，第 6 个会无法区分颜色），请先取消一个再选。」；底部计数显示「已选 N / 5 个 · 同单位多指标并排」。
  - 单位拦截：同屏多指标必须单位一致，不一致时**直接不勾选**并提示「同屏多指标必须单位一致：已选「工时」（h），本次选「里程」（km），请换一个同单位指标。」
  - 单指标替换：选中新指标即替换原有的，并提示「只支持 1 个指标：「工时」已替换为「能耗」」；「全选当前模型」在该类组件下隐藏，多选组件点全选超限时提示「该组件最多选 5 个指标，全选会超限，请逐个点选。」。
  - 连带规则：切换可视化类型时按新类型的上限裁剪（数据明细 3 列 → 机群折线只留第 1 个并提示）；读取已保存的版式 JSON 时同样按「组件类型 × 口径」重新裁剪。
- 弹窗标题随场景变化（`从模型字典选择` / `选择明细列（可多选）` / `添加指标卡`）。
- 底部：`已选 N 项`（指标卡场景为 `已选 N 项 · 指标卡已加 N / 6 张`；数据明细为 `已选 N 列 · 一列一个指标（明细表可多选）`）、`取消全选` / `全选当前模型`（仅数据明细显示）、`取消` / `确定`；弹窗内以红色提示条承载超限与单位冲突提示，避免被弹窗遮罩挡住。

#### 3.1.2.5 校验与查看 JSON

- 校验（设计器内实时校验并回显结论）：
  - 未通过：画布下方弹出黄色告警条 `⚠ 校验未通过：<最多 4 条问题>… 等`，问题如「当前口径（机群）画布至少需要 1 个组件」「第 N 个组件缺少组件标题」「第 N 个组件（柱状图）未绑定指标」「未选择统计时间维度」。
  - 通过：**不显示任何提示**；校验条只在有问题时出现。
...
| 指标数量 | 折线图 / 环形图：机群口径 = 1 个、单机口径 ≤ 5 个（须同单位）；柱状图：两个口径均 ≤ 5 个（须同单位）；指标卡 ≤ 6 张（一个字段一张卡）；数据明细 ≥ 1 列、不限列数 |
