...
| 5 | **报表整合** | 单机与机群报表整合为**同一张** |
| 6 | **指标接入深度** | 丰富矿卡、挖掘机、履带吊的指标接入深度 |
| 7 | **指标计算口径** | 从一律求和，丰富为按字段登记聚合方式（求和 / 重算 / 期末快照 / 差值 / 最大值 / 去重计数 / 不呈现） |
| 8 | **PDF 附件配置** | 可视化版式设计器（区块编排 + 变量绑定 + 实时预览），不保留 Word 模板方式 |
| 9 | **PDF 预览** | 列表 / 详情弹窗内嵌 PDF 预览；设计器实时预览 |
| 10 | **报表需求征集** | PC 端与 App 端支持用户报表需求征集：勾选候选报表或填写自定义诉求，提交后由后台统一处理 |
...
本期新增 **7 张**报表（单机与机群同一张：同一份字段与版式，按口径渲染两种视图）：
...
- 筛选工具栏：`模版名称`（占位「输入模版名称...」，回车即触发搜索）、`语言类型`（`全部语言` + 15 种语言）、`搜索` / `重置`。
- 数据列表区：7 列表格（图标 / 模版名称 / 模版描述 / 语言 / 启用 / 最近更新 / 操作）；列表为空时展示空态文案「无匹配模版」。
...
- 不再有「适用范围」「报告维度」「PDF 版式」三列：适用范围由附件步骤自动汇总，报告维度收敛为设备维度，PDF 版式仅在详情与版式 JSON 中体现。
...
- 步骤 2 XLSX / CSV 附件：`文件名模板`（支持变量，占位「如：{{模版名称}}_{{发送日期}}」）、口径分段切换 `单机设备` / `机群（聚合）`、对应口径的数据列配置；仅本步可跳过。
- 步骤 3 PDF 附件：`文件名模板` + PDF 版式设计器。
- 弹窗底部：`启用此模版`开关（所有步骤固定可见）、`跳过此步`（仅第 2 步且未跳过时显示，文案「跳过此步（不生成 XLSX/CSV 附件）」）、`恢复此步`、`上一步` / `下一步：XLSX / CSV 附件`（第 2 步为「下一步：PDF 附件」，第 3 步隐藏）、`完成并保存`（仅第 3 步显示）。
...
- 触发方式：点击列表的「删除」操作。
- 弹窗内容：标题 `确认删除此模版？`，正文「删除后将无法恢复，相关订阅任务也会同步失效。」。
- 操作：`取消` / `确认删除`。
...
- 必填与校验：模版名称、描述、正文必填；未通过时拦截并提示具体原因（`模版名称不能为空` / `模版描述不能为空` / `正文内容不能为空`）。
- 语言与适用范围：编辑已有模版时语言不可改；适用范围由附件步骤的口径配置自动派生（第 2、3 步都跳过时默认两种口径都支持），管理员不能手工增删；v1.2 的「模版类型单选」「适用范围勾选」均已取消，界面不再有勾选控件。
- 适用范围汇总口径：第 2、3 步中发生过多口径操作的口径计入适用范围。
- 数据列配置归属：单机配置与机群配置各存一套；未进入某个步骤时不得覆盖该步骤已保存的配置，保证两套配置互不冲掉。
- 跳过与恢复：仅第 2 步可跳过；跳过后该步配置不参与保存；可点「恢复此步」把配置找回。

边界值：

- 列表 7 列、无分页；至少保留 1 个模版（最后一条的删除入口置灰）。
...
- 设计器顶部：
  ...
  - 统计时间维度：`日报` / `周报` / `月报` 三个勾选，默认三项全选，可取消（至少保留一项）。
...
- 基础形态：`组件标题`（文本，必填）+ `可视化类型`（6 种下拉，见下表）。
- 柱状图排布方向（仅柱状图）：两枚互斥按钮 `▥ 垂直（时间序列）` / `≡ 水平（设备排行）`，自动适配长文本标签。
- 数据透视与维度：指标行列表（`第 N 张 · 指标名` + 行内「✖」移除）+ `＋ 从模型字典选择`（图表 / 环形图 / 数据明细）或 `＋ 添加指标卡`（指标卡）；未绑定字段时提示「⚠ 尚未绑定字段，画布无法出图」；指标卡满 6 张时提示「已达上限 6 张，先移除一张再加。」
- PDF 物理排版：`纸张宽度占比`（`100%（满宽整行）` / `50%（半宽并排）`）、`渲染完毕后强制换页`（勾选框）、`🗑 删除当前组件`。
- 文本段落（仅文本段落）：`段落内容`（多行文本，占位「输入报表的文字说明、导言或总结分析...」）。

| 可视化类型 | 指标绑定规则 | 说明 |
| --- | --- | --- |
| 折线图 | 1 个指标 | X 轴为时间序列；单机一条线，机群每台设备一条线（机群只允许同指标对比） |
| 柱状图 | 1 个指标 | 排布方向可切垂直（时间序列）/ 水平（设备排行，全量不截断、数值降序） |
| 环形图 | 1 个指标 | 单机 = 同维度状态构成（工作 / 怠速 / 关机等）；机群 = 单机贡献度（前 5 台 + 第 6 个「其他」中性灰） |
| 指标卡 | ≤ 6 张 | 一个字段一张卡；每次只选 1 个字段、确认后追加一张；已加过的指标在选择列表内置灰 |
| 数据明细 | 可多选（一个指标一列） | 行维度为时间 / 设备；多列同屏时要求单位一致 |
| 文本段落 | 不需要绑定 | 用于口径说明、导言或总结 |
...
- 三栏级联：`设备类型`（14 种设备类型，机群口径的物模型列改为「报表」）→ `物模型` → `字段`；字段列右侧显示单位徽标；表头随场景显示「字段（单选 1 个）」/「字段（可多选 · 一个指标一列）」/「字段（每次选 1 个 · 加 1 张卡）」，右上角显示字段个数。
- 选择控件：图表 / 环形图 / 指标卡为单选框（再点一次可取消）；数据明细为复选框；基础字段「统计周期 / 设备机型 / 设备编号」始终置灰不可选，指标卡中已加过的字段同样置灰。
- 底部：`已选 N 项 · 字段来源：报表模版数据词典`、`取消全选` / `全选当前模型`（仅数据明细显示）、`取消` / `确定`；弹窗内以红色提示条承载超限与单位冲突提示，避免被弹窗遮罩挡住。

#### 3.1.2.5 校验与查看 JSON

- 校验（设计器内实时校验并回显结论）：
  - 未通过：`⚠ 校验未通过：<最多 4 条问题>… 等`，问题如「当前口径（机群）画布至少需要 1 个组件」「第 N 个组件缺少组件标题」「第 N 个组件（柱状图）未绑定指标」「未选择统计时间维度」。
  - 通过：`✓ 校验通过（单机口径）：N 个组件，半数宽度 M 个，时间维度 日报 / 周报 / 月报`。
- 查看 json：先执行校验，未通过则仅滚动到校验条并中止；通过后写入当前模版的版式 JSON（随模版保存落库）并弹出窗口，提供 `复制 JSON`（提示 `JSON 已复制`）。

#### 3.1.2.6 指标单位一致性（防错设计）
...
- 绑定校验：折线图 / 柱状图 / 环形图至少且只能绑定 1 个指标；指标卡 ≤ 6 张且不得重复；数据明细至少 1 列且表头必填；组件标题必填；环形图各项占比合计需在 99.8%–100.2%；变量取不到值时显示 `--`。
- 单位校验：同屏（同一组件 + 同一报表）指标单位必须一致；冲突指标只提示不自动撤销，需管理员自行更换。

边界值：

| 边界 | 值 |
| --- | --- |
| 可视化类型 | 6 种：折线图 / 柱状图 / 环形图 / 指标卡 / 数据明细 / 文本段落（无饼状图，已下线并自动迁移为环形图） |
| 指标数量 | 折线图 / 柱状图 / 环形图 = 1 个；指标卡 ≤ 6 张（一个字段一张卡）；数据明细 ≥ 1 列 |
| 字段字典 | 14 类设备类型、34 个物模型；基础字段 3 个（统计周期 / 设备机型 / 设备编号） |
| 单位体系 | 油车 `L` / `L·h⁻¹`；电车自动换算 `kWh`；跨机型数值单位冲突拦截 |
| 排版 | 纸张宽度占比 100% / 50% 两档；是否强制换页按组件单独设置；A4 纵向 |
| 统计时间维度 | 日报 / 周报 / 月报，默认三项全选，至少 1 项 |
...
- 报表清单（**暂不实现**）：按分组展示，每组标题右侧显示「N 张报表」；无结果显示「未找到匹配的报表」。

#### 3.2.1.3 列表功能规范

- 状态标签：已有标签的 6 张 —— `已上线`（红）3 张 = 设备能耗 / 设备工作模式 / 设备移动；`已计划`（蓝）3 张 = 超时工作 / 设备异常通知 / 电池电量。
...
- 视图与格式切换：`单机` / `机群` 口径分段切换；`表格` / `PDF` 两种预览格式切换（PDF 为版式效果示意，内容为同一份示例数据）。
...
- 交互闭环：底部 `关闭` / `使用该报表`（→ 跳转「管理报表」页）；点关闭、点遮罩、按 Esc 均可关闭。
...
功能入口：管理报表页右上角「添加报表」→ 宽版模态弹窗，标题「添加报表」，右上角关闭。
...
- 报表类型：上方 `单机` / `机群` 左右 Tab + 下方「定期报表类型」下拉（Tab 决定下拉内容）；选中类型后展示类型说明副文案，类型与资产数量联动（单机类型限选 1 台）。
- 类型清单：
  - 单机 7 项：设备利用率报表、设备运行快照报表、设备燃油效率报表、设备怠速分析报表、设备事件统计报表、设备事件历史报表、单机月度 360° 报表。
  - 机群 6 项：机群作业状态报表、机群状态清单报表、机群利用率报表、机群怠速与利用率报表、机群燃油效率报表、位置历史报表。
  - 项目维度 5 项（项目生产报表（按行程）、项目效率报表、项目能耗报表、项目安全报表、项目设备清单报表）随清单保留但入口不开放、暂不可选。
- 资产穿梭框：
  ...
  - 未选择报表类型时，资产穿梭框不展开。
- 维度说明：弹窗内不出现维度 Tab，统一为「报表类型 + 资产穿梭框」；项目维度的选择面板与专属配置保留结构但隐藏。
- 项目维度面板（暂不开放，结构保留）：4 个来源 Tab（所有项目（默认）/ 工地现场 / 在地理围栏中 / 在群组中）；双栏面板左栏为搜索 + 排序（A→Z / Z→A / 最近使用）+ 多选项目列表（带设备数徽章），右栏为「已选项目」+「全部移除 (N)」+ 二次检索 + 按来源分组折叠；底部计数「已选 N 个项目 · 覆盖 M 台设备」实时刷新。
...
> 三个数据源（oT 平台 / 业务系统 / 物模型）经**数据接入与清洗**后，依次经过**统计日切分 → 字段口径注册表 → 指标聚合引擎**，产出**统一结果集**，再适配到手机端 / PC 图表 / PDF / 邮件四端。其中**行程事件与 T1–T5 时间戳、装载/卸载点坐标与海拔**，以及**项目实体、工地现场、地理围栏、设备群组**为新增来源；物模型指标范围与「设备管理 → 设备详情 → 统计报表」一致。
